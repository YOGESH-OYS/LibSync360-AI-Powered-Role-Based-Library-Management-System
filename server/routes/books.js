...
const { semanticSearch, getBookRecommendations } = require('../services/aiService');
...
router.get('/', optionalAuth, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const sortField = sortBy === 'title' ? 'title' : 
                     ...
                     sortBy === 'rating' ? 'averageRating' : 'popularityScore';
...
router.post('/', verifyToken, requireStaff, [
  body('title', 'Title is required').notEmpty().isLength({ max: 200 }),
  body('author', 'Author is required').notEmpty().isLength({ max: 100 }),
  body('isbn', 'Valid ISBN is required').matches(/^(?:ISBN(?:-1[03])?:? )?(?=[0-9X]{10}$|(?=(?:[0-9]+[- ]){3})[- 0-9X]{13}$|97[89][0-9]{10}$|(?=(?:[0-9]+[- ]){4})[- 0-9]{17}$)(?:97[89][- ]?)?[0-9]{1,5}[- ]?[0-9]+[- ]?[0-9]+[- ]?[0-9X]$/),
...
router.delete('/:id', verifyToken, requireStaff, asyncHandler(async (req, res) => {
  const { id } = req.params;
...
router.get('/search/semantic', async (req, res) => {
  try {
    ...
    const result = await semanticSearch(query, { limit: parseInt(limit), filters });
