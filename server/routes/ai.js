        ...
        $addFields: {
          trendScore: {
            $add: [
              ...
              { $multiply: [{ $subtract: ['$totalCopies', '$availableCopies'] }, 0.1] }
            ]
          }
        }
      },
...
router.get('/predict-overdue/:borrowingId', authenticateToken, async (req, res) => {
  try {
...
router.post('/generate-embeddings', [authenticateToken, require('../middleware/auth').authorizeRoles(['admin'])], async (req, res) => {
  try {
    ...
    if (bookId) {
      ...
      for (const book of booksWithoutEmbeddings) {
        try {
          ...
          logger.error(`Error generating embedding for book ${book._id}:`, error);
        }
      }
...
router.get('/insights', [authenticateToken, require('../middleware/auth').authorizeRoles(['admin'])], async (req, res) => {
  try {
...
router.get('/academic-recommendations', authenticateToken, async (req, res) => {
  try {
    ...
    if (!user || user.role !== 'student') {
      return res.status(400).json({ message: 'Only students can get academic recommendations' });
    }
