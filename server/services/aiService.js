      ...
      messages: [
        {
          role: "system",
          content:
            "You are a helpful library search assistant. Provide relevant search terms for finding books.",
        },
        {
          role: "user",
          content: prompt,
...
const getBookRecommendations = async (userId, options = {}) => {
  ...
  try {
    ...
    const genres = [
      ...new Set(userBorrowings.flatMap((b) => b.book.genre || [])),
    ];
    const subjects = [
      ...new Set(userBorrowings.flatMap((b) => b.book.subjects || [])),
    ];
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a knowledgeable librarian. Recommend books based on user preferences.",
        },
        {
          role: "user",
          content: prompt,
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a sentiment analysis expert. Analyze text and return structured JSON responses.",
        },
        {
          role: "user",
          content: prompt,
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a library analytics expert. Predict overdue probability based on various factors.",
        },
        {
          role: "user",
          content: prompt,
...
const generateBookEmbedding = async (book) => {
  ...
  try {
    const text = `${book.title} ${book.author} ${
      book.description || ""
    } ${book.genre?.join(" ")} ${book.subjects?.join(" ")}`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a book embedding generator. Convert book information into numerical representation.",
        },
        {
          role: "user",
