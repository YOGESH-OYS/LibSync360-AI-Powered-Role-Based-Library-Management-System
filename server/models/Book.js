...
bookSchema.virtual('availabilityPercentage').get(function() {
  return this.totalCopies > 0 ? (this.availableCopies / this.totalCopies) * 100 : 0;
});
...
bookSchema.statics.searchBooks = async function(query, filters = {}) {
  ...
  // Apply filters
  if (filters.genre) {
    searchQuery.genre = { $in: Array.isArray(filters.genre) ? filters.genre : [filters.genre] };
  }
  
  if (filters.subjects) {
    searchQuery.subjects = { $in: Array.isArray(filters.subjects) ? filters.subjects : [filters.subjects] };
  }
...
bookSchema.methods.updatePopularityScore = async function() {
  ...
  this.popularityScore = (recentBorrowings * 0.4) + (this.averageRating * 0.3) + (this.totalRatings * 0.3);
  return await this.save();
};
