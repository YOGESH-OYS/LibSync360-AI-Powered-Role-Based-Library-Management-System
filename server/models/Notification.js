...
const notificationSchema = new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: ['lend', 'return', 'reminder', 'overdue', 'fine', 'system', 'review'],
    required: true
  },
  title: {
    type: String,
...
notificationSchema.statics.getUserNotifications = async function(userId, filters = {}) {
...
notificationSchema.methods.markDeliveryFailed = async function(method, errorMessage) {
  const existingMethod = this.sentVia.find(v => v.method === method);
  if (existingMethod) {
