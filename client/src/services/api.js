...
export const booksAPI = {
  ...
  getRecommendations: (limit) => api.get('/books/recommendations', { params: { limit } }),
  updateAvailability: (id, change) => api.put(`/books/${id}/availability`, { change }),
};
...
export const finesAPI = {
  ...
  updateStatus: (id, status, notes) => api.put(`/fines/${id}/status`, { status, notes }),
...
// AI API
export const aiAPI = {
  search: (query, limit, filters) => api.get('/ai/search', { params: { query, limit, filters } }),
  getRecommendations: (limit, type) => api.get('/ai/recommendations', { params: { limit, type } }),
  getTrending: (limit, period) => api.get('/ai/trending', { params: { limit, period } }),
  analyzeSentiment: (text) => api.post('/ai/sentiment', { text }),
  predictOverdue: (borrowingId) => api.get(`/ai/predict-overdue/${borrowingId}`),
  getSimilar: (bookId, limit) => api.get(`/ai/similar/${bookId}`, { params: { limit } }),
  generateEmbeddings: (bookId) => api.post('/ai/generate-embeddings', { bookId }),
